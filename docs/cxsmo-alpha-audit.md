# C✦SMO Transparent Asset Audit

## Existing candidates reviewed

The local `cxsmo-starlight-shell-alpha-fallback.png` is a tall, full-garment black moto shell cutout with the full collar, sleeves, hem, and a clear transparent surround. It is suitable as a Starlight Moto Shell product-layer candidate.

The local `cxsmo-key-charm-alpha-fallback.png` is a full silver four-point-star key charm cutout, including its ring and clasp, with a clear transparent surround. It is suitable as an Orbit Key Charm product-layer candidate.
