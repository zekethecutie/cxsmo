...
describe("C✦SMO entry, studio, and account expansion", () => {
  ...
  it("keeps screenshot-matched hero layering, a trigger-anchored clickable menu, and a centered entry poster contract", () => {
    ...
    expect(entry).toContain("repeat: Infinity");
	  ...
	  expect(entry).toContain('className="cxsmo-entry__mark-star" aria-hidden="true" />SMO</motion.h1>');
	  expect(entryLandingRebuild).toContain("clip-path:polygon(50% 0");
	  expect(entryLandingRebuild).toContain("font-size:inherit");
	  expect(entryLandingRebuild).toContain("overflow:visible");
