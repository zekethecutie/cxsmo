...
|---|---|---|
| Primary navigation | The menu opens by click or `ArrowDown`, focuses its first link after keyboard opening, closes on outside pointer interaction or `Escape`, and returns focus to its trigger on dismissal. | The menu is a non-modal disclosure; it does not intentionally trap focus. |
