...
export function CxsmoHomePage() {
  ...
  <section className="cxsmo-service-strip"><div><Sparkles size={21} /><p><b>Everything has an information layer.</b> Fit notes, saved objects, bag states, and shipping-request previews are available in this portfolio experience.</p></div><Link href="/cxsmo/support">Explore the info desk <ArrowUpRight size={16} /></Link></section></CxsmoShell>;
}
...
export function CxsmoSupportPage() {
  ...
  return <CxsmoShell><section className="cxsmo-page-intro cxsmo-page-intro--compact"><p className="section-label">Information desk</p><h1>Need a<br /><em>little clarity?</em></h1><p>Product and service information should feel as considered as the clothes.</p></section><section className="cxsmo-info-list">{items.map(([id, title, body], index) => <article className={open === id ? "is-open" : ""} key={id}><button aria-expanded={open === id} onClick={() => setOpen(open === id ? "" : id)}><span>0{index + 1}</span><b>{title}</b><Plus size={20} /></button>{open === id && <p>{body}</p>}</article>)}</section></CxsmoShell>;
}
