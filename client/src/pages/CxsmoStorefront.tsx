...
export function CxsmoShopPage() {
  ...
  return <CxsmoShell><section className="cxsmo-page-intro"><p className="section-label">C✦SMO objects</p><h1><TextReveal text="Wear the" /><br /><em>whole signal.</em></h1><p>A fictional drop of volume-first streetwear, made to demonstrate an editorial product-discovery path.</p></section><section className="cxsmo-shop-toolbar"><label className="cxsmo-shop-search"><Search size={17} /><span className="sr-only">Search catalogue</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the drop" /></label><div className="cxsmo-filter-scroll" aria-label="Product categories">{cxsmoCategories.map((category) => <button key={category} className={filter === category ? "is-active" : ""} onClick={() => setFilter(category)}>{category}</button>)}</div><label>Sort <select value={sort} onChange={(event) => setSort(event.target.value)}><option>Featured</option><option>Price: low</option><option>Price: high</option></select><ChevronDown size={15} /></label></section><section className="cxsmo-product-grid cxsmo-product-grid--shop" aria-live="polite">{products.map((product, index) => <ProductCard product={product} order={`0${index + 1}`} key={product.id} />)}</section>{!products.length && <section className="cxsmo-empty-state cxsmo-search-empty"><Search size={24} /><h2>No signal found.</h2><p>Try a product name, category, colour, or drop number. This search only reads the twelve fictional catalogue objects in the browser.</p><button className="cxsmo-button" type="button" onClick={() => { setQuery(""); setFilter("All"); }}>Reset discovery</button></section>}<section className="cxsmo-collection-note"><p>Not seeing a category?</p><h2>Accessories and beauty<br />are staged for the next release.</h2><span>This concept intentionally presents only the fictional product data above.</span></section></CxsmoShell>;
}
