...
export function CxsmoLegalPage() { return <CxsmoShell><section className="cxsmo-legal"><p className="section-label">Portfolio terms / version 01</p><h1>Clear terms.<br /><em>No theatre.</em></h1><div><article><h2>Portfolio disclosure</h2><p>C✦SMO is a fictional fashion-commerce interface designed and developed by zxke for presentation purposes. Products, inventory, orders, receipts, shipping, reviews, and customer states are simulated unless explicitly labelled otherwise.</p></article><article><h2>Privacy and local data</h2><p>Profile, bag, style-preference, and checkout-preview entries are stored only in this browser when you use the local demo controls. No payment, contact, address, or order information is submitted to C✦SMO.</p></article><article><h2>Terms of use</h2><p>This experience is display-only. It does not form a sales offer, delivery agreement, or product claim. Do not enter sensitive personal information into a portfolio demonstration.</p></article></div></section></CxsmoShell>; }

type LegalSurfaceProps = { eyebrow: string; title: [string, string]; sections: { heading: string; copy: string }[] };
function CxsmoLegalSurface({ eyebrow, title, sections }: LegalSurfaceProps) { return <CxsmoShell><section className="cxsmo-legal"><p className="section-label">{eyebrow}</p><h1>{title[0]}<br /><em>{title[1]}</em></h1><div>{sections.map((section) => <article key={section.heading}><h2>{section.heading}</h2><p>{section.copy}</p></article>)}</div></section></CxsmoShell>; }
export function CxsmoPrivacyPage() { return <CxsmoLegalSurface eyebrow="Privacy / browser-local demo" title={["Your data.", "Your device."]} sections={[{ heading: "Browser-local preferences", copy: "C✦SMO uses browser-local storage for demo bag, saved-object, appearance, sound, profile, and checkout-preview preferences. Those states exist to demonstrate interface behavior and are not sent to a C✦SMO customer database." }, { heading: "No transaction data", copy: "C✦SMO does not collect payment, delivery, contact, order, fulfilment, or verified-customer information through this portfolio presentation. Do not enter sensitive personal information." }, { heading: "Map preview", copy: "If you choose to preview a location, the map appears only to illustrate a possible delivery interface. The address field remains a browser-session demonstration input." }]} />; }
export function CxsmoTermsPage() { return <CxsmoLegalSurface eyebrow="Terms / presentation use" title={["Use the", "demo clearly."]} sections={[{ heading: "Display-only experience", copy: "C✦SMO is not a storefront, sales offer, delivery agreement, product claim, or financial transaction flow. All displayed prices, stock, commerce states, and operations are fictional presentation material." }, { heading: "Appropriate use", copy: "Use this interface to evaluate interaction design, art direction, portfolio architecture, and visual systems. Do not rely on it for a purchase, shipping commitment, beauty claim, or inventory availability." }, { heading: "Content boundary", copy: "Portfolio-safe placeholders, honest empty states, and original fictional media are used to demonstrate a future-capable system without fabricating customers, reviews, orders, or endorsements." }]} />; }
export function CxsmoDisclosurePage() { return <CxsmoLegalSurface eyebrow="Disclosure / zxke portfolio" title={["Built to", "be pitched."]} sections={[{ heading: "Portfolio authorship", copy: "C✦SMO is a fictional fashion and lifestyle demonstration designed and developed by zxke to show high-end ecommerce, content-management, motion, and operations design capabilities." }, { heading: "Fictional commerce", copy: "The product catalogue, operations studio, customer-service states, and checkout interactions are illustrative. They do not represent a live business, customer base, merchant of record, or transaction processor." }, { heading: "Media and interactions", copy: "The campaign and product media are original C✦SMO concept assets. Audio feedback is optional, starts only after visitor interaction, and can be disabled at any time." }]} />; }
