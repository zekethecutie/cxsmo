...
const scenes: TourScene[] = [
  { code: "00", route: "/cxsmo", eyebrow: "C✦SMO / signal in", title: ["C✦SMO", "FOR THE", "AFTER-IMAGE."], copy: "A black, bone, and signal-red world that makes the first second feel like a drop, not a page load.", target: "HERO / SIGNAL IN", cursor: [65, 56], scrollY: 0, cue: "theme" },
  { code: "01", route: "/cxsmo/shop", eyebrow: "Object scan / inventory", title: ["THE DROP", "IS A", "WORLD."], copy: "Twelve fictional objects are staged like collectable frames: sharp hierarchy, fast recognition, zero transaction theatre.", target: "OBJECT GRID / CUTOUT", cursor: [55, 61], scrollY: 260, cue: "shutter" },
  { code: "02", route: "/cxsmo/products/gravity-01", eyebrow: "Product dossier / focus", title: ["VOLUME", "IS THE", "HOOK."], copy: "A product page moves from object to attitude, with the fit, detail, and decision point framed in one clean shot.", target: "PRODUCT STAGE / ZOOM", cursor: [71, 58], scrollY: 0, cue: "chapter" },
  { code: "03", route: "/cxsmo/edits", eyebrow: "Editorial motion / styling", title: ["WORN", "TO MOVE", "THE FEED."], copy: "A timed lookbook turns isolated clothes into a repeatable visual rhythm: outfit, signal, then the next hit.", target: "LOOKBOOK / MORPH", cursor: [59, 46], scrollY: 160, cue: "chapter" },
  { code: "04", route: "/cxsmo/account", eyebrow: "Local profile / choice", title: ["YOUR", "SIGNAL.", "YOUR CALL."], copy: "Taste, fit, locale, and recommendations stay browser-local so the experience demonstrates commerce logic without pretending to own a customer.", target: "ACCOUNT / LOCAL ONLY", cursor: [47, 55], scrollY: 240, cue: "success" },
  { code: "05", route: "/cxsmo/checkout", eyebrow: "Checkout / boundary", title: ["STAGE", "THE", "FLOW."], copy: "Address preview, delivery hierarchy, and consent make the interface feel real while the portfolio boundary stays in full view.", target: "CHECKOUT / CONSENT", cursor: [63, 48], scrollY: 0, cue: "click" },
  { code: "06", route: "/cxsmo/admin", eyebrow: "Operator mode / studio", title: ["SHAPE", "THE", "SIGNAL."], copy: "The campaign, content, product presentation, and promotional moments stay editable—because the pitch is a system, not a static skin.", target: "STUDIO / CONTROL ROOM", cursor: [73, 43], scrollY: 0, cue: "open" },
  { code: "07", route: "/cxsmo", eyebrow: "C✦SMO / designed by zxke", title: ["THIS IS", "C✦SMO."], copy: "A fictional fashion-commerce film built to prove how a product world can look, move, sound, and sell an idea before it ever sells a product.", target: "PORTFOLIO / END FRAME", cursor: [67, 39], scrollY: 0, cue: "finish", outro: true },
];
...
export function CxsmoTourProvider({ children }: { children: ReactNode }) {
  ...
  const nextShot = useCallback(() => { setPlaying(false); goToScene(sceneIndex === scenes.length - 1 ? 0 : sceneIndex + 1); }, [goToScene, sceneIndex]);
